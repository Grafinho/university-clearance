...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    $id = (int)$_POST['id'];
    $status = $_POST['status'];
    $comments = $_POST['comments'];   // You can use this as "Admin Reply"

    $stmt_update = $conn->prepare("UPDATE messages SET status=?, comments=? WHERE id=?");
    $stmt_update->bind_param("ssi", $status, $comments, $id);
...
// ====================== FETCH RECORDS ======================
if ($is_non_student) {
    // Non-students see all messages
    $sql = "SELECT * FROM messages ORDER BY created_at DESC";
    $stmt = $conn->prepare($sql);
} else {
    // Regular users see only their own messages
    $sql = "SELECT * FROM messages WHERE username = ? ORDER BY created_at DESC";
    $stmt = $conn->prepare($sql);
...
?>
    ...
    <head>
        ...
        <title>Manage Messages - University Clearance System</title>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <form method="POST">
                                ...
                                <td>
                                    <textarea name="comments" rows="3"><?= htmlspecialchars($row['comments'] ?? '') ?></textarea>
                                </td>
