...
if (isset($_POST["submit"])) {
    ...
    if ($file && in_array($fileType, $allowedTypes)) {
        // Convert file to BLOB
        $fileData = file_get_contents($file);

        // Prepare SQL statement
        $stmt = $conn->prepare("INSERT INTO clearance_documents (student_id, student_name, document_type, file_data, file_type, file_size) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssi", $student_id, $student_name, $document_type, $fileData, $fileType, $fileSize);
        ...
        echo "<script>alert('Invalid file type or no file selected.');</script>";
    }
}
