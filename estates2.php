...
// HANDLE UPDATE (ESTATES CORRECT FIELDS)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {

    $stmt_update = $conn->prepare("UPDATE estates 
        SET hostel_name=?, room_no=?, pending_arrears=?, status=?, comments=? 
        WHERE id=?");
...
if ($is_non_student) {
    ...
    $sql = "SELECT * FROM estates WHERE username = ?";
    $stmt = $conn->prepare($sql);
                        ...
                        <form method="POST">
                            ...
                            <td>
                                <input type="text" name="hostel_name"
                                       value="<?= htmlspecialchars($row['hostel_name'] ?? '') ?>">
                            </td>

                            <td>
                                <input type="text" name="room_no"
                                       value="<?= htmlspecialchars($row['room_no'] ?? '') ?>">
                            </td>

                            <td>
                                <input type="number" step="0.01" name="pending_arrears"
                                       value="<?= $row['pending_arrears'] ?>">
                            </td>
