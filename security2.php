...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $update_sql = "UPDATE security 
                   SET offence=?, critical=?, fine_amount=?, status=?, comments=? 
                   WHERE id=?";
...
if ($is_non_student) {
    ...
    $sql = "SELECT * FROM security WHERE username = ?";
    $stmt = $conn->prepare($sql);
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <form method="POST">
                            ...
                            <td>
                                <input type="text" name="offence" value="<?= htmlspecialchars($row['offence'] ?? '') ?>">
                            </td>

                            <td>
                                <select name="critical">
                                    <option value="No" <?= ($row['critical']=='No')?'selected':'' ?>>No</option>
                                    <option value="Yes" <?= ($row['critical']=='Yes')?'selected':'' ?>>Yes</option>
                                </select>
                            </td>

                            <td>
                                <input type="number" step="0.01" name="fine_amount" value="<?= $row['fine_amount'] ?>">
                            </td>
